...
use crate::{
    domain::{
        ...
        Capital, Liquidity, UserBucket, UserBuckets, UserCash, UserId,
    },
    error::ParsingError,
    log::{
...
/// Парсер капитала (провалидированного через [Capital::new])
#[derive(Debug, Clone)]
pub struct ParseCapital;
impl<'a> Parser<'a> for ParseCapital {
    type Dest = Capital;
    ...
    fn parse(
        ...
        let (remaining, count) = NonZeroU32::MIN.parse(input)?;
        Ok((remaining, Capital::new(count)?))
    }
}
/// Конструктор [ParseCapital]
fn capital() -> ParseCapital {
    ParseCapital
}
/// Парсер ликвидности (провалидированной через [Liquidity::new])
#[derive(Debug, Clone)]
pub struct ParseLiquidity;
impl<'a> Parser<'a> for ParseLiquidity {
    type Dest = Liquidity;
    ...
    fn parse(
        ...
        let (remaining, count) = NonZeroU32::MIN.parse(input)?;
        Ok((remaining, Liquidity::new(count)?))
    }
}
/// Конструктор [ParseLiquidity]
fn liquidity() -> ParseLiquidity {
    ParseLiquidity
}
...
impl Parsable for UserCash {
    type Parser = Map<
        Delimited<
            All<(StripWhitespace<Tag>, StripWhitespace<Tag>)>,
            Permutation<(KeyValue<ParseUserId>, KeyValue<ParseCapital>)>,
            StripWhitespace<Tag>,
        >,
        fn((UserId, Capital)) -> Self,
    >;
...
mod test {
    ...
    fn test_log_kind() {
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal RegisterAsset {"asset_id": "bayc", "liquidity": 100000000, "user_id": "Steeve",}"#), Ok(("", LogKind::App(AppLogKind::Journal(AppLogJournalKind::RegisterAsset{asset_id: "bayc".parse().unwrap(), user_id: "Steeve".parse().unwrap(), liquidity: Liquidity::new(NonZeroU32::new(100_000_000).unwrap()).unwrap()})))));
        assert_eq!(
            LogKind::parser().parse(
            ...
            Ok((
                ...
                LogKind::App(AppLogKind::Journal(AppLogJournalKind::DepositCash(
                    UserCash::new(
                        "Steeve".parse().unwrap(),
                        Capital::new(NonZeroU32::new(10).unwrap()).unwrap()
                    )
                )))
            ))
        );
    ...
    #[test]
    fn test_capital_rejects_below_minimum() {
        assert_eq!(capital().parse("9"), Err(ParsingError::CapitalTooLow));
        assert!(capital().parse("10").is_ok());
    }
        ...
        assert_eq!(
            ...
            Ok((
                ...
                AppLogJournalKind::RegisterAsset {
                    ...
                    user_id: "Steeve".parse().unwrap(),
                    liquidity: Liquidity::new(NonZeroU32::new(100).unwrap()).unwrap(),
                }
            ))
        );
    }
    ...
    #[test]
    fn test_liquidity_rejects_below_minimum() {
        assert_eq!(liquidity().parse("49"), Err(ParsingError::LiquidityTooLow));
        assert!(liquidity().parse("50").is_ok());
    }
    ...
    #[test]
    fn test_log_line_parser_journal() {
        let (remaining, line) = LogLine::parser()
            .parse(
    ...
    fn test_log_line_trailing_garbage_is_reported_as_remaining() {
        ...
        let (remaining, _) = LogLine::parser()
